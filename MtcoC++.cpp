...
int main()
{
    ...
    double area_react = y_upper * (x_upper - x_lower);
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        // this check is unecessary since all points are bound be in this range
        if (x >= x_lower && x <= x_upper)
        {
            ...
            {
                monte_area += 1;
                result = (double)monte_area / total_area * area_react;
            }
        }
    }
    if (total_area != 0)
    {
        result = area_react * monte_area / total_area;
    }
